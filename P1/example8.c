...
int main (int argc, char ** argv) {
    ...
    int etiqueta = 0;
    int getiqueta = 0;
    int etiquetas[MAX_ETIQUETAS]; int cima_etiquetas=-1;
    FILE * fd_asm;
    ...
    //Condición del if anidado. Salto a fin si, si la condicion se da. 
    ...
    //Salto a fin sino al terminar el if anidado, impresion de la etiqueta fin_si. Recogemos la etiqueta.
    etiqueta = etiquetas[cima_etiquetas]; 
    ifthenelse_fin_then(fd_asm, etiqueta);
    
    //printf 3, else anidado
    escribir_operando(fd_asm,"6",0); 
    escribir(fd_asm,0,ENTERO);
